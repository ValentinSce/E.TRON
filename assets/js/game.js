...
ctx.fillStyle = 'white';
ctx.fillText('Click to play', 375, 32);
...
var gameOver = function() {
	if (!player1.alive) {
		if (!player2.alive) {
			...
			result = 'Jaune tu as gagné';
		}
	} else {
		result = 'Marron tu as Gagné';
	}
	playing = false;
	ctx.fillStyle = 'white';
	ctx.fillText(result + '. Click pour rejouer', 350, 32);
};
...
var update = function () {
	...
	if (90 in keysDown) { // Player 1 pressed up
		player1.dx = 0;
		player1.dy = -1 * player1.speed;
	}
	if (83 in keysDown) { // Player 1 pressed down
		player1.dx = 0;
		player1.dy = player1.speed;
	}
	if (81 in keysDown) { // Player 1 pressed left
		player1.dx = -1 * player1.speed;
		player1.dy = 0;
	}
	if (68 in keysDown) { // Player 1 pressed right
		player1.dx = player1.speed;
		player1.dy = 0;
	}
	if (38 in keysDown) { // Player 2 pressed up
		player2.dx = 0;
		player2.dy = -1 * player2.speed;
	}
	if (40 in keysDown) { // Player 2 pressed down
		player2.dx = 0;
		player2.dy = player2.speed;
	}
	if (37 in keysDown) { // Player 2 pressed left
		player2.dx = -1 * player2.speed;
		player2.dy = 0;
	}
	if (39 in keysDown) { // Player 2 pressed right
